...
class Fish {
     ...
     */
    async generateFish() {
        const fish = await db.select('*')
                             .from(TABLE_NAME)
                             .whereRaw(`id = abs(random()) % (SELECT max(id) FROM ${TABLE_NAME}) + 1`)
                             .limit(1);
        ...
        this.length    = Math.round((this.length + Number.EPSILON) * 100) / 100;
    ...
    async catch(userId) {
        ...
        const points           = Math.ceil(this.length * pointMultiplayer);
